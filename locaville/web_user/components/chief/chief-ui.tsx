...
export interface Farmer {
  ...
  /** 거주 주소(시·군·면·리). amo_family.addr_1 우선, 없으면 user_master.addr_1. */
  address?: string;
}
...
export interface InboxItem {
  ...
  evidence?: {
    ...
    when: string;
    img: string;
    /** 촬영 후 AI 가 To-do 일치를 확신 못함 → 이장님 확인 필요 (backend needs_chief_verification). */
    needsChiefVerification?: boolean;
    /** AI 판정 사유 (todo_match_reason). */
    matchReason?: string;
    /** 영수증 OCR — 있을 때만 (영수증 사진). 이장님이 사진 내용 검증용. */
    receiptOcr?: {
      vendor?: string;
