...
2. `.github/workflows/deploy-web_user.yml` 예시:

```yaml
name: Deploy web_user to Vercel
  ...
  deploy:
    ...
    steps:
      ...
      - name: Deploy to Vercel (prod)
        ...
        run: |
          # monorepo: --cwd points to the app root
          vercel --token $VERCEL_TOKEN --prod --confirm --cwd=locaville/web_user
```

Notes:
- Use `--cwd` to target the app folder inside a monorepo. `--prod` deploys to production immediately; remove it for Preview deployments.
- If you prefer the official Vercel Action or third-party actions, you can swap the last step with that action; the CLI approach is lightweight and explicit.
